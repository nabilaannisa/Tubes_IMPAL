...
public class Aplikasi {
    ...
    public static Pegawai getPgw() {
        return pgw;
    }

    public static void setPgw(Pegawai p) {
        Aplikasi.pgw = p;
    }
    
//    public void savePegawai(Pegawai p){
//        Database db = new Database();
//        String s = "insert into pegawai values ('" + pgw.getIdPegawai() + "','" + pgw.getNamaPegawai() + "','" + pgw.getAlamat() + "','" 
//                + pgw.getNoHP() + "','" + pgw.getUsername() + "','" + pgw.getPass()+ "')" ;
//        db.query(s);
//    }
    ...
    public Pegawai getPegawai(){
        ...
        try {
            ...
            rs = db.getData(s);
            while(rs.next()){
                p = new Pegawai(rs.getString(1), rs.getString(2));
                p.setUsername(rs.getString(3));
                p.setPwdPegawai(rs.getString(4));
            }   
    ...
    public Pegawai getPegawaiUser(String user, String pass){
        Database db = new Database();
        Pegawai p = null;
        ResultSet rs;
        try {
            String s = "select*from pegawai where username = '" + user + "' and password= '" + pass + "'" ;
            rs = db.getData(s);
            while(rs.next()){
                p = new Pegawai(rs.getString(1), rs.getString(2));
                p.setUsername(rs.getString(3));
